# -*- coding: utf-8 -*-
"""
Created on Fri Nov 18 16:04:41 2022
...
from helpfun import fill_diag, row_op, com_del, targets_del, revisit


#index of stars
sInds = np.arange(1973)
...
while tc<len(tf):
    if i ==0:
        ...
        while ko == 0:
            ...
            ko = np.prod(koMap[steps[0],tc.item():intime.item(steps[0])])*np.prod(koMap[steps[1],tc.item()+intime.item(steps[0]):intime.item(steps[1])])
                
            if ko == 0:
                B = targets_del(B,steps[0],steps[1])
            order = np.reshape(np.array([A_i,steps[0],steps[1]]),(1,3))
        
        tc = tc+intime[steps[0]]+intime[steps[1]] # adding integration time to current time
    
        #create a null matrix for keeping tab of number of observations done    
        
        Null = np.zeros((1,len(sInds))) #initial matrix for keeping count of number of observations done
        
        revisit_0 = revisit(len(sInds),Null,A_i)+revisit(len(sInds),Null,steps[0])+revisit(len(sInds),Null,steps[1])
        ...
        while ko2 == 0 and tc<len(tf):
            ...
            ko2 = np.prod(koMap[step[0],tc.item():intime.item(step[0])])*np.prod(koMap[step[1],tc.item()+intime.item(step[0]):intime.item(step[1])])
                
            if ko2 == 0:
                C = targets_del(C,step[0],step[1])
                
            #orders = np.append(order,step[0])
            #o1 = np.append(orders,step[1])
            G = np.append(G,step)
            ko2 = 0
            tc = tc+intime[step[0]]+intime[step[1]] # adding integration time to current time
